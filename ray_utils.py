...
Rays = collections.namedtuple('Rays', ('origins', 'directions', 'viewdirs', 'radius', 'lossmult', 'near', 'far'))
...
def convert_to_ndc(origins, directions, focal, w, h, near=1.):
    ...
    t = -(near + origins[..., 2]) / (directions[..., 2] + eps)
    ...
    # Projection
    o0 = -((2 * focal) / w) * (ox / (oz + eps))
    o1 = -((2 * focal) / h) * (oy / (oz + eps) )
    o2 = 1 + 2 * near / (oz+ eps)

    d0 = -((2 * focal) / w) * (dx / (dz + eps) - ox / (oz + eps))
    d1 = -((2 * focal) / h) * (dy / (dz + eps) - oy / (oz + eps))
    d2 = -2 * near / (oz+ eps)
...
def lift_gaussian(d, t_mean, t_var, r_var, diag):
    ...
    Returns
    ---
    - diag=True: mean, cov [batch_size, num_sample, 3]
    """
    mean = d[..., None, :] * t_mean[..., None] # (N, num_sample, 3)
    
    d_mag_sq = torch.sum(d ** 2, dim=-1, keepdim=True) + 1e-10 # (N, 3)
    
    if diag:
        d_outer_diag = d ** 2
        null_outer_diag = 1 - d_outer_diag / d_mag_sq
        t_cov_diag = t_var[..., None] * d_outer_diag[..., None, :] # (N, num_sample, 3)
        xy_cov_diag = r_var[..., None] * null_outer_diag[..., None, :]
        cov_diag = t_cov_diag + xy_cov_diag
        return mean, cov_diag
    else:
        d_outer = d[..., :, None] * d[..., None, :]                         # (N, 3, 3)
        eye = torch.eye(d.shape[-1], device=d.device)                       # I_{3*3} 
        null_outer = eye - d[..., :, None] * (d / d_mag_sq)[..., None, :]
        t_cov = t_var[..., None, None] * d_outer[..., None, :, :]           # (N, num_sample, 3)
        xy_cov = r_var[..., None, None] * null_outer[..., None, :, :]
        cov = t_cov + xy_cov
        return mean, cov        
    
    
def conical_frustum_to_gaussian(directions, t0, t1, base_radius, diag, stable=True):
    """Approximate a conical frustum as a Gaussian distribution (mean+cov) 
        Assume the ray is originating from the origin, and base_radius is the
    radius at dist=1. Doesn't assume `d` is normalized.
    
    Arguments:
    ---
        directions: [batch_size, 3]
        t0: float: starting endpoint
        t1: float: ending endpoint
        base_radius: radius at dist=1
        diag: boolean if True, only return the diagonal covariance matrix
        stable: boolean, whether or not to use the stable computation described in
      the paper (setting this to False will cause catastrophic failure e.g. NAN).
    ...
    if stable:
        ...
        raise RuntimeError("not recommended")
    return lift_gaussian(directions, t_mean, t_var, r_var, diag)
...
def cast_rays(t_vals, origins, directions, radii, ray_shape, diag=True):
    ...
    Arguments:
    ---
      t_vals: float array, [batch_size, num_samples], the "fencepost" distances along the ray. 
      origins: float array, [batch_size, 3], the ray origin coordinates.
      directions: float array, [batch_size, 3], the ray direction vectors.
      radii: float array, [batch_size, TODO: ]the radii (base radii for cones) of the rays.
      ray_shape: cone or cylineder
    ...
    if ray_shape == 'cone':
        ...
        raise ValueError(f"ray_shape not support {ray_shape}")
...
def sample_along_rays(origins, directions, radii, num_samples, near, far, randomized, lindisp, ray_shape):
    ...
    ---
      - origins: 
        ...
        shape: [batch_size, 3],
        description: ray origins.
      - directions: 
        type: torch.tensor(float32), 
        shape: [batch_size, 3], 
        description: ray directions.
        
      - radii:
        type: torch.tensor(float32), 
        shape: [batch_size, 3], 
        description: ray radii.  
      - num_samples: int.
    ...
    if randomized:
        mids = 0.5 * (t_vals[..., 1:] + t_vals[..., :-1]) # [t_i, t_{i+1}]  mid = (t_i + t_{i+1}) / 2
...
def volumetric_rendering(rgb, density, t_vals, dirs, white_bkgd):
    ...
    disp: torch.tensor(float32), [batch_size], depth esitimation
    acc: torch.tensor(float32), [batch_size].
    weights: torch.tensor(float32), [batch_size, num_samples], weights of the corresponding sampling color
    alpha:
    """
    # print(rgb.shape, density.shape, t_vals.shape, dirs.shape, white_bkgd)
    # exit(0)
    t_mids = 0.5 * (t_vals[..., 1:] + t_vals[..., :-1])
    t_dists = t_vals[..., 1:] - t_vals[..., :-1]                    # (batch_size, num_samples)
    delta = t_dists * torch.linalg.norm(dirs[..., None, :], dim=-1) # (batch_size, num_samples)
    
    density_delta = density[..., 0] * delta                 #  (batch_size, num_sample)
    ...
    weights = alpha * trans                                 #  (batch_size, num_sample)
    comp_rgb = (weights[..., None] * rgb).sum(dim=-2)       #  (batch_size, 3)
